...
VOTING_SAMPLES, VOTING_NUM_COLS = 303, 14

VOTING_FEATS = ['handicapped-infants', 'water-project-cost-sharing',
...
VOTING_TARGET = "Class"
VOTING_CLASSES = ["democrat", "republican"]
...
def process_heart(df_in, answer_mapping=DEF_MAPPING, scaler=None, ret_xy=False):
    ...
    arguments:
        ...
        answer_mapping...   custom answer mapping, otherwise using default
                            mapping: {"y": 1, "n": -1, "unknown": 0}. Can also
                            be turned of by passing None or False
        scaler...           instance of scaler to use (e.g. StandardScaler())
    """
    df = df_in.copy()
    try: 
        df.rename(columns={"class": VOTING_TARGET}, inplace=True)
    except:
    ...
    if scaler:
        df[VOTING_FEATS] = scaler.fit_transform(df[VOTING_FEATS]) 

    # Add more as needed...
    if ret_xy:
        return df[VOTING_FEATS], df[VOTING_TARGET]
    return df
